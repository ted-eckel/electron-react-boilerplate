...
import { updateNote, openNote, updateNoteMeta, archiveNote, trashNote } from '../actions/files/notes'
...
const mapDispatchToProps = dispatch => bindActionCreators({
  trashFiles, /* should take an array of filePaths */
  archiveFiles, /* should take an array of filePaths */
...
class FileListItem extends Component {
  ...
  render() {
    ...
    const editorState = EditorState.createWithContent(convertFromRaw(((
      item.meta && item.meta.content
    ) ? htmlConvert(item.meta.content) : htmlConvert(''))))

    // const editorState = ((item.meta && item.meta.content
    // ) ?
    //   createEditorState(convertFromRaw(mediumDraftImporter(item.meta.content))) :
    //   createEditorState()
    // )
        ...
        <Paper style={{ width: '240px', padding: '12px 0', backgroundColor: colorHex(colorState) }}>
          <div style={{ cursor: 'default' }} onClick={toggleCreateNoteModal}>
            ...
            <div style={{
              ...
              {/* <Editor editorState={editorState} editorEnabled={false} /> */}
            </div>
          </div>
          <TagsInput
            onlyUnique
